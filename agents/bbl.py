...
class BBL(Agent):
    ...
    def reset(self):
        ...
        self.probs_prod = [1] * len(self.experts.generators)
        self.agent_probs = [1] * len(self.experts.generators)
        self.priors = np.array([1 / len(self.experts.generators)] * len(self.experts.generators))
        self.next_round_num = 1
        self.expert_to_use = random.choice(self.experts.generators)
        self.prev_reward = 0
        self.log_message = ''

    def _record_opp_actions_demo(self, round_num, opp_reward):
        agents = list(self.experts.generators)
    ...
    def _update_expert(self, round_num):
        def _expected_val(agent, h=1):
            opp_agents = list(self.experts.generators)
            expected_reward = 0
        ...
        agents = self.experts.generators

        best_agent_idx, best_reward = None, -np.inf
        ...
        for i, agent in enumerate(agents):
            ...
            if expected_reward > best_reward:
                best_agent_idx, best_reward = i, expected_reward

        if self.log:
            self.log_message += f'{best_agent_idx}'

        self.expert_to_use = self.experts.generators[best_agent_idx]
