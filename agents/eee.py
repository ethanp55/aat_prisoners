...
class EEE(Agent):
    def __init__(self, name: str, game, player: int, log=False, demo=False):
        Agent.__init__(self, name=name, actions=[])

        self.name = name
        self.agents = GeneratorPool('pool', game, player)
        self.player = player
        self.log = log
        self.demo = demo
        self.log_message = ''
        ...
        self.phase_rewards = []
        self.n_i = 0
        self.agent_in_use = random.choice(self.agents.generators)
        self.explore_prob = np.random.choice([0.05, 0.06, 0.07, 0.08, 0.09, 0.10]) if not self.demo else 0.50
        self.agent_names = [gen.name for gen in self.agents.generators]
    ...
    def reset(self):
        ...
        for expert in self.agents.generators:
        ...
        if round_num >= self.prev_round:
            ...
            if not self.in_phase:
                ...
                if explore:
                    ...
                    new_agent = random.choice(self.agents.generators)
                    ...
                    for key, val in self.m_e.items():
                        if val == max_reward:
                            agents_to_consider.append(self.agents.generators[self.agent_names.index(key)])
