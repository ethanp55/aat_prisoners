...
class SPP(Agent):
    def __init__(self, name: str, game: MarkovGameMDP, player: int, lmbda=0.99, n_simulations=10, log=False):
        ...
        self.log_message = ''
        self.rewards = []
        self.experts = GeneratorPool('pool', self.game, self.player)
        self.expert_names = [gen.name for gen in self.experts.generators]
        self.expert_agents = self.experts.generators
        self.minimax_val = self.experts.generators[self.expert_names.index('MinimaxGenerator')].generator.v_p1[str(self.game.get_init_state())] if self.player == P1 else self.experts.generators[self.expert_names.index('MinimaxGenerator')].generator.v_p2[str(self.game.get_init_state())]
    ...
    def _run_simulation(self, agent1, agent2, n_rounds):
        ...
        for _ in range(self.n_simulations):
            ...
            for r_num in range(n_rounds):
                ...
                reward1 = 0
                reward2 = 0
                agent_map = {agent1.name: agent1, agent2.name: agent2} if agent1.generator.player == P1 \
                    else {agent2.name: agent2, agent1.name: agent1}

                while not curr_state.is_terminal():
                    for agent_name, agent in agent_map.items():
                        agent_reward = prev_reward_1 if agent.generator.player == P1 else prev_reward_2
                        action_p1, action_p2 = agent.act(curr_state, agent_reward, r_num)
                        action_map[agent_name] = action_p1 if agent.generator.player == P1 else action_p2
                    ...
                    for agent_name, new_reward in updated_rewards_map.items():
                        if agent_map[agent_name].generator.player == P1:
                            reward1 += new_reward
    ...
    def _get_potential(self, n_rounds=10):
        ...
        for expert in self.expert_agents:
            if expert.name == 'CoopGenerator':
                agent1 = deepcopy(expert)
                agent2 = deepcopy(self.experts.generators[self.expert_names.index('CoopGenerator')])
                agent2.player, agent2.name = 1 - self.player, 'CoopGenerator1'

                potential = self._run_simulation(agent1, agent2, n_rounds)

                potentials.append(potential)

            elif expert.name == 'CoopPunishGenerator':
                agent1 = deepcopy(self.experts.generators[self.expert_names.index('CoopGenerator')])
                agent2 = deepcopy(self.experts.generators[self.expert_names.index('CoopGenerator')])
                agent2.player, agent2.name = 1 - self.player, 'CoopGenerator1'

                potentials.append(self._run_simulation(agent1, agent2, n_rounds))

            # elif expert.name == 'BullyGenerator':
            #     agent1 = deepcopy(expert)
            #     agent2 = deepcopy(self.experts.generators[self.expert_names.index('BullyGenerator')])
            #     agent2.player, agent2.name = 1 - self.player, 'BullyGenerator1'
            #
            #     potential = self._run_simulation(agent1, agent2, n_rounds)
            #
            #     potentials.append(potential)

            elif expert.name == 'BullyPunishGenerator':
                agent1 = deepcopy(expert)
                agent2 = deepcopy(self.experts.generators[self.expert_names.index('BulliedGenerator')])
                agent2.player = 1 - self.player

                potentials.append(self._run_simulation(agent1, agent2, n_rounds))

            elif expert.name == 'BulliedGenerator':
                agent1 = deepcopy(expert)
                agent2 = deepcopy(self.experts.generators[self.expert_names.index('BullyPunishGenerator')])
                agent2.player = 1 - self.player
            ...
            elif expert.name == 'MinimaxGenerator':
                potentials.append(self.minimax_val)

            elif expert.name == 'CFRGenerator':
                potentials.append(0)
    ...
    def act(self, state, reward, round_num):
        if self.should_update_expert and round_num > self.prev_change_round:
            ...
            if len(filtered_expert_set) == 0:
                filtered_expert_set.append('CFRGenerator')
            ...
            if old_expert != self.expert_to_use and isinstance(self.experts.generators[self.expert_names.index(self.expert_to_use)], FolkEgalPunishAgent):
                self.experts.generators[self.expert_names.index(self.expert_to_use)].start_round, self.experts.generators[self.expert_names.index(self.expert_to_use)].should_attack = \
                    round_num + 1, False

            if self.log:
                self.log_message = f'{round_num};{self.aspiration};{row};{r_bar_t};{f_prob};{use_curr_expert};{filtered_expert_set};{self.expert_to_use}'

        return self.experts.generators[self.expert_names.index(self.expert_to_use)].act(state, reward, round_num)

    def reset(self):
        self.rewards = []
        for expert in self.experts.generators:
